...
mod kind_workflow_tests {
    ...
    fn test_init_intent_flow_creates_config_with_kinds() {
        ...
        for kind in ["intent", "invariant", "contract", "algorithm", "evaluation", "pace", "monitor"] {
            assert!(
    ...
    fn test_kind_add_custom() {
        ...
        ctx.cmd()
            .args(["kind", "add", "deployment", "--description", "Deployment configuration and constraints"])
            .assert()
            .success()
    ...
    fn test_kind_remove_nonexistent() {
        ...
        // Nonexistent kind should report not found regardless of --force
        ctx.cmd()
    ...
    fn test_add_artifact_with_registered_kind() {
        ...
        ctx.cmd()
            .args(["add", "evaluation", "Given valid user, when login, then session created"])
            .assert()
            .success()
    ...
    fn test_add_artifact_with_custom_kind() {
        ...
        ctx.cmd()
            .args(["add", "deployment", "Must deploy to us-east-1 with 3 replicas"])
            .assert()
            .success()
            .stdout(predicate::str::contains("deployment"));
    }
    ...
    fn test_search_filters_by_kind() {
        ...
        ctx.cmd()
            .args(["add", "intent", "User login authentication flow"])
            .assert()
            .success();
        ctx.cmd()
            .args(["add", "evaluation", "Login success criteria test"])
            .assert()
            .success();

        // Search within intent kind only
        ctx.cmd()
            .args(["search", "login", "--kind", "intent"])
            .assert()
            .success()
    ...
    fn test_full_intent_flow_workflow() {
        ...
        let artifacts = [
            ...
            ("algorithm", "Password hashing uses bcrypt with cost factor 12"),
            ("evaluation", "Given valid credentials, when login, then token returned"),
        ...
        ctx.cmd()
            .args(["kind", "add", "deployment", "--description", "Deployment rules"])
            .assert()
            .success();

        ctx.cmd()
            .args(["add", "deployment", "Deploy to us-east-1 with min 3 replicas"])
            .assert()
            .success();
        ...
        // Step 6: Verify total artifact count
        ctx.cmd()
            .args(["list"])
            .assert()
            .success();
